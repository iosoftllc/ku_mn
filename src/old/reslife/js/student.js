...
function sortList(val) {
	...
	document.StudentForm.action = "../../src/apply/stu_list.php";
...
function viewStudent(no) {
	if (no != "") {
		...
		document.StudentForm.action = "../../src/apply/stu_view.php";
		document.StudentForm.submit();
	}
}
...
function postStudent(mode) {
	...
	document.StudentForm.action = "../../src/apply/stu_post.php";
	document.StudentForm.submit();
}
...
function checkStudentInfo(Form) {
	...
	if (Form.kind[1].checked) {
		...
		if (Form.id.value.length < 9) {
			alert("학번은 반드시 9자리 이상이여야 합니다.");
			Form.id.focus();
			return false;
		}
	}
...
function deleteStudent(no) {
	...
	if (no == "") alert("선택하신 학생이 없습니다.");
		...
		if (flag) {
			...
			document.StudentForm.action = "../../src/apply/stu_action.php";
			document.StudentForm.submit();
		}
	}
}
...
function downloadExcel() {
	if (!document.StudentForm.purpose.value) {
		...
		document.StudentForm.action = "../../src/apply/stu_excel.php";
		document.StudentForm.submit();
	}
}
...
function showPhotoList() {
	if (!document.StudentForm.purpose.value) {
		...
		document.StudentForm.action = "../../src/popup/stu_photo.php";
		document.StudentForm.submit();
	}
}
...
function checkDuplicateEmail(Form) {
	...
	var URL = "../../src/popup/check_email.php?email=" + Form.email.value;
