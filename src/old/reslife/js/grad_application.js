...
function refreshPeriodList(code) {
	var urlValue = "../../src/graduate/app_select.php?type=rate&code=" + code;
...
function updatePeriodList() {
	...
	if (document.getElementById("rate1") != null) {
		...
		for (var i = 0; i < period.length; i++) {
			...
			optionObj.appendChild(document.createTextNode(period[i].getElementsByTagName("name")[0].childNodes[0].nodeValue + " " + period[i].getElementsByTagName("price")[0].childNodes[0].nodeValue));
			document.getElementById("rate1").appendChild(optionObj);
		}
	}
	optionObj = null;
...
function sortList(val) {
	...
	document.ApplicationForm.action = "../../src/graduate/app_list.php";
...
function viewApplication(no) {
	if (no != "") {
		...
		document.ApplicationForm.action = "../../src/graduate/app_view.php";
		document.ApplicationForm.submit();
	}
}
...
function postApplication(mode) {
	...
	document.ApplicationForm.action = "../../src/graduate/app_post.php";
	document.ApplicationForm.submit();
}
...
function submitPeriod(Form) {
	if (checkPeriodInfo(Form)) {
		Form.mode.value = "add";
		Form.method = "post";
		Form.action = "../../src/graduate/app_action.php";
		Form.submit();
	}
}
...
function deleteApplication(no) {
	...
	if (no == "") alert("선택하신 페이지가 없습니다.");
		...
		if (flag) {
			...
			document.ApplicationForm.action = "../../src/graduate/app_action.php";
			document.ApplicationForm.submit();
		}
	}
}
...
function approveApplication(Form, flag) {
	if (Form.no.value) {
		...
		if (answer) {
			...
			Form.method = "post";
			Form.action = "../../src/graduate/app_action1.php";
			Form.submit();
		}
	}
}
...
function downloadExcel() {
	if (!document.ApplicationForm.purpose.value) {
		...
		document.ApplicationForm.action = "../../src/graduate/app_excel.php";
		document.ApplicationForm.submit();
	}
}

function downloadExcel1() {
	document.ApplicationForm.action = "../../src/graduate/app_excel1.php";
	document.ApplicationForm.submit();
}
...
function showPhotoList() {
	if (!document.ApplicationForm.purpose.value) {
		...
		document.ApplicationForm.action = "../../src/popup/grad_app_photo.php";
		document.ApplicationForm.submit();
	}
}
...
function checkEmailInfo(Form) {
	...
	var URL = "../../src/popup/grad_email_info.php?email=" + Form.email.value;
...
function printDepositReceipt(no) {
	if (no) {
		if (popup != null && !popup.closed) popup.close();
		var URL = "../../src/popup/grad_receipt_d.php?no=" + no;
...
function printHallReceipt(no) {
	if (no) {
		if (popup != null && !popup.closed) popup.close();
		var URL = "../../src/popup/grad_receipt_h.php?no=" + no;
		popup = window.open(URL, '_receipt','resizable=no,scrollbars=yes,status=no,width=650,height=650');
	}
}
...
function printInvoice(no) {
	if (no) {
		if (popup != null && !popup.closed) popup.close();
		var URL = "../../../v1/src/popup/grad_invoice.php?no=" + no;
...
function gotoMonth(Form, yr, mth) {
	if (yr && mth) {
		...
		Form.action = "../../src/graduate/app_calendar.php";
		Form.submit();
	}
}
...
function viewContract(no) {
	if (no) {
		if (popup != null && !popup.closed) popup.close();
		var URL = "../../../v1/src/popup/grad_contract.php?no=" + no;
