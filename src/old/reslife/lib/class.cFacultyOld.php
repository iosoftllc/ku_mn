<?
	...
	class cFaculty extends cMysql {
		...
		function getDormitoryCode($val) {
			...
			else if ($val == "ISRH_AN") $returnValue = "ANAM";
			else $returnValue = "";
			return $returnValue;
		}
		...
		function getDormitoryValue1($val) {
			...
			else if ($val == "ISRH_AN") $returnValue = "International Student Residence Halls - Anam";
			else $returnValue = "";
			return $returnValue;
		}
		...
		function getDormitoryValue2($val) {
			...
			else if ($val == "ISRH_CJ") $returnValue = "CJ";
			else if ($val == "IFRH_FH") $returnValue = "IFH";
			else if ($val == "ISRH_AN") $returnValue = "ANAM";
			else $returnValue = "";
			return $returnValue;
		...
		function getFacultyCondition($flag, $dorm, $sdate, $edate, $stype, $stext, $term, $state, $grade, $rate, $room) {
			...
			if ($dorm) {
				...
				for ($i = 0; $i < count($arr_temp); $i++) {
					if ($arr_temp[$i] == "CJI") $list_dorm .= "'STUDIO1','DOUBLE','FAMILY','TRIPLE','QUAD','QUINT','OCTET','HANDICAP',";
					else if ($arr_temp[$i] == "IFH") $list_dorm .= "'STUDIO2','1BED','2BED','DEPART',";
				}
				if ($list_dorm) $list_dorm = substr($list_dorm, 0, strlen($list_dorm) - 1);
			}
			if ($list_dorm) {
				if ($returnValue) $returnValue .= " AND a.rate_code IN ($list_dorm)";
				else $returnValue .= " WHERE a.rate_code IN ($list_dorm)";
			}
			if ($term == "S") {
