<?
	...
	class GraduateRefund extends cMysql {
		...
		function GraduateRefund($host, $id, $pw, $db, $tbl1, $tbl2, $tbl3, $tbl4, $tbl5, $tbl6, $tbl7) {
		...
		function getWhereCondition($sdate, $edate, $stype, $stext, $kind, $new, $approve, $period) {
			...
			$returnValue = "";
			if ($kind) $returnValue = "WHERE e.kind='$kind'";
			if ($new == "C") {
		...
		function getRefundList($sdate, $edate, $start, $size, $stype, $stext, $kind, $new, $approve, $period, $sort) {
			...
			$this->clearSQL();
			$this->appendSQL("SELECT refund_no, a.apply_no, approve, e.kind, info_no, a.student_id, a.fname, a.mname, a.lname, a.email, vacate_dt, edit_dt, post_dt, approve_dt, b.name, a.room_code, new_period, method_type ");
		...
		function getExcelList($sdate, $edate, $stype, $stext, $kind, $new, $approve, $period, $sort) {
			...
			while (!$this->EOF) {
				...
				$this->listApprove[$cnt] = $this->getField("approve");
				$this->listKind[$cnt] = $this->getField("e.kind");
		...
		function getRefundInfo($no) {
			$this->clearSQL();
			$this->appendSQL("SELECT * FROM $this->refundTableName a LEFT JOIN $this->periodTableName b ON a.old_period=b.period_code LEFT JOIN $this->periodTableName c ON a.new_period=c.period_code ");
			$this->appendSQL("LEFT JOIN $this->applyTableName d ON a.apply_no=d.apply_no LEFT JOIN $this->studentTableName e ON d.email=e.email WHERE refund_no=$no");
			$this->parseQuery();
			if (!$this->EOF) {
				...
				$this->refundKind = $this->getField("e.kind");
		...
		function insertRefund($apply, $cf_apply, $student, $fname, $mname, $lname, $dob, $email, $vacate, $method, $info1, $info2, $info3, $info4, $info5, $info6, $line2, $line3, $city, $state, $postal, $country, $room, $old, $new, $admin) {
			global $ihouse_admin_info;
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->refundTableName (apply_no, cf_apply_no, student_id, fname, mname, lname, dob, email, vacate_dt, post_dt, method_type, method_info1, method_info2, method_info3, method_info4, method_info5, method_info6, addr1, addr2, city, state, postal, country, room_code, old_period, new_period, admin)");
			$this->appendSQL("VALUES ($apply, $cf_apply, '$student', '$fname', '$mname', '$lname', '$dob', '$email', '$vacate', now(), '$method', '$info1', '$info2', '$info3', '$info4', '$info5', '$info6', '$line2', '$line3', '$city', '$state', '$postal', '$country', '$room', '$old', '$new', '$admin')");
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', 'O', 'N', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			return $returnValue;
		...
		function updateRefund($no, $apply, $cf_apply, $student, $fname, $mname, $lname, $dob, $email, $vacate, $method, $info1, $info2, $info3, $info4, $info5, $info6, $line2, $line3, $city, $state, $postal, $country, $room, $old, $new, $admin) {
			global $ihouse_admin_info;
			$this->clearSQL();
			$this->appendSQL("UPDATE $this->refundTableName SET apply_no=$apply, cf_apply_no=$cf_apply, student_id='$student', fname='$fname', mname='$mname', lname='$lname', ");
			$this->appendSQL("dob='$dob', email='$email', vacate_dt='$vacate', method_type='$method', method_info1='$info1', method_info2='$info2', method_info3='$info3', method_info4='$info4', method_info5='$info5', method_info6='$info6', ");
			$this->appendSQL("addr1='$line2', addr2='$line3', city='$city', state='$state', postal='$postal', country='$country', room_code='$room', ");
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', 'O', 'E', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			return $returnValue;
		...
		function deleteRefund($no) {
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', 'O', 'D', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
 			return $returnValue;
