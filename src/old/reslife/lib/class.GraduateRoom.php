<?
	...
	class GraduateRoom extends cMysql {
		var $roomTableName;
		...
		function GraduateRoom($host, $id, $pw, $db, $tbl) {
		...
		function getRateName($rate) {
			$returnValue = "";
			if (trim($rate) == "G_SINGLE") $returnValue = "Single";
			else if (trim($rate) == "G_DOUBLE") $returnValue = "Double";
			else if (trim($rate) == "G_TRIPLE") $returnValue = "Triple";
			return $returnValue;
		}

		function getRoomCondition($rate, $stext, $stype) {
			$returnValue = "FROM $this->roomTableName WHERE rate_code LIKE 'G_%'";
		...
		function getRoomCount($rate, $stext, $stype) {
			$this->clearSQL();
			$this->appendSQL("SELECT COUNT(*) AS cnt " . $this->getRoomCondition($rate, $stext, $stype));
			$this->parseQuery();
		...
		function getRoomList($rate, $start, $size, $stext, $stype, $sort) {
			if ($sort == "") $sort = " ORDER BY room_code";
			else $sort = " ORDER BY $sort";
			if ($start != "" || $start == "0") $limit = " LIMIT $start, $size";
			else $limit = "";
			$this->clearSQL();
			$this->appendSQL("SELECT * " . $this->getRoomCondition($rate, $stext, $stype) . $sort . $limit);
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
