<?
	...
	class cRoom extends cMysql {
		...
		function getRateName($rate) {
			$returnValue = "";
			if (trim($rate) == "SINGLE") $returnValue = "CJ I'House - Single";
			else if (trim($rate) == "DOUBLE1") $returnValue = "CJ I'House/Anam Global - Double";
			else if (trim($rate) == "TRIPLE") $returnValue = "Anam Global - Triple";
			else if (trim($rate) == "ANAM") $returnValue = "ANAM2 - Triple";
			return $returnValue;
		}

		function getRoomCondition($rate, $stype, $stext) {
			$returnValue = "FROM $this->roomTableName WHERE rate_code IN ('SINGLE','DOUBLE1','TRIPLE','ANAM')";
