<?
	...
	class GraduateStudent extends cMysql {
		...
		function GraduateStudent($host, $id, $pw, $db, $tbl1, $tbl2, $tbl3, $tbl4, $tbl5) {
			$this->studentTableName = $tbl1;
			$this->accountTableName = $tbl2;
		...
		function getKindValue($val){
			if ($val == "G") $returnValue = "일반";
			else if ($val == "S") $returnValue = "특수";
			else $returnValue = "";
			return $returnValue;
		}
		...
		function getStudentCondition($sdate, $edate, $stype, $stext, $kind, $nation, $current) {
			$returnValue = "";
			if ($kind) {
				if ($returnValue) $returnValue .= " AND kind='$kind'";
				else $returnValue .= " WHERE kind='$kind'";
			}
			if ($nation) {
				if ($returnValue) $returnValue .= " AND nationality LIKE '%" . $nation . "%'";
				else $returnValue .= " WHERE nationality LIKE '%" . $nation . "%'";
			}
			if ($current) {
		...
		function getStudentCount($sdate, $edate, $stype, $stext, $kind, $nation, $current) {
			$this->clearSQL();
			$this->appendSQL("SELECT COUNT(*) AS cnt " . $this->getStudentCondition($sdate, $edate, $stype, $stext, $kind, $nation, $current));
			$this->parseQuery();
		...
		function getStudentList($sdate, $edate, $start, $size, $stype, $stext, $kind, $nation, $current, $sort) {
			if ($sort == "") $sort = " ORDER BY post_date DESC";
			else $sort = " ORDER BY $sort";
			if ($start != "" || $start == "0") $limit = " LIMIT $start, $size";
			else $limit = "";
			$this->clearSQL();
			$this->appendSQL("SELECT * " . $this->getStudentCondition($sdate, $edate, $stype, $stext, $kind, $nation, $current) . $sort . $limit);
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
				...
				$this->stuListProvince[$cnt] = $this->getField("province");
		...
		function updatePassword($no, $pw) {
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', 'S', 'E', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			return $returnValue;
		...
		function insertStudent($pw, $kind, $current, $id, $fnm, $mnm, $lnm, $pr_nm, $nm_kr, $gender, $dob, $nation, $province, $kgsp, $uni, $addr, $addr2, $addr3, $city, $state, $postal, $country, $addr1, $major, $class, $email, $ph, $cell, $case_nm, $case_rel, $case_ph, $case_addr, $admin) {
			global $ihouse_admin_info;
			$pw = crypt($pw);
			$acc = "";
			/*
			...
			*/
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->studentTableName (passwd, kind, current, student_id, fname, mname, lname, prefer, name_kr, gender, dob, nationality, province, kgsp, home_uni, home_addr, home_addr1, home_addr2, home_city, home_state, home_postal, home_country, res_addr, major, class, email, phone, cell, case_name, case_relate, case_phone, case_addr, account, post_date, admin)");
			$this->appendSQL("VALUES ('$pw', '$kind', '$current', '$id', '$fnm', '$mnm', '$lnm', '$pr_nm', '$nm_kr', '$gender', '$dob', '$nation', '$province', '$kgsp', '$uni', '$addr', '$addr2', '$addr3', '$city', '$state', '$postal', '$country', '$addr1', '$major', '$class', '$email', '$ph', '$cell', '$case_nm', '$case_rel', '$case_ph', '$case_addr', '$acc', now(), '$admin')");
			$returnValue = $this->execQuery();
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', 'S', 'N', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			if ($returnValue && $acc) {
		...
		function updateStudent($no, $kind, $current, $id, $fnm, $mnm, $lnm, $pr_nm, $nm_kr, $gender, $dob, $nation, $province, $kgsp, $uni, $addr, $addr2, $addr3, $city, $state, $postal, $country, $addr1, $major, $class, $email, $ph, $cell, $case_nm, $case_rel, $case_ph, $case_addr, $admin) {
			global $ihouse_admin_info;
			$old = "";
			$this->clearSQL();
			$this->appendSQL("SELECT email FROM $this->studentTableName WHERE info_no=$no");
			...
			$this->appendSQL("mname='$mnm', lname='$lnm', prefer='$pr_nm', name_kr='$nm_kr', gender='$gender', dob='$dob', nationality='$nation', province='$province', kgsp='$kgsp', home_uni='$uni', ");
			$this->appendSQL("home_addr='$addr', home_addr1='$addr2', home_addr2='$addr3', home_city='$city', home_state='$state', home_postal='$postal', ");
			$this->appendSQL("home_country='$country', res_addr='$addr1', major='$major', class='$class', email='$email', phone='$ph', cell='$cell', ");
		...
		function deleteStudent($no) {
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', 'S', 'D', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			if ($returnValue && $acc) {
		...
		function insertHistoryWork($menu, $kind, $detail) {
			...
			if (strtolower($admin_id) != "intia") {
				$ip = $_SERVER["REMOTE_ADDR"];
				$this->clearSQL();
				$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'P', '$menu', '$kind', '$ip', now(), '$detail')");
				$this->execQuery();
			}
		}
	}
