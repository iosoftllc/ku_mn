<?
	...
	class GraduateApplication extends GraduateStudent {
		...
		var $preRateSDate = array();
		var $preRateEDate = array();
		...
		function getPeriodList1($no, $email, $date) {
			...
			$this->appendSQL("LEFT JOIN $this->priceTableName c ON a.rate_code=c.rate_code AND a.period_code=c.period_code WHERE apply_no<>'$no' AND email='$email' AND sdate>='$date' ORDER BY apply_no DESC");
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
		...
		function getRateList($period) {
			$this->clearSQL();
			$this->appendSQL("SELECT a.rate_code, a.name rate_nm, b.price, b.sdate, b.edate FROM $this->rateTableName a ");
			$this->appendSQL("LEFT JOIN $this->priceTableName b ON a.rate_code=b.rate_code WHERE b.period_code='$period' ORDER BY a.order_no");
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
				...
				$this->rateSDate[$cnt] = $this->getField("sdate");
				$this->rateEDate[$cnt] = $this->getField("edate");
				$this->setNextRecord();
				$cnt++;
			}
			$this->freeResult();
		...
		function getPreferenceList($no, $period) {
			...
			$this->preRateSDate = null;
			$this->preRateEDate = null;
			$this->preRatePrice = null;
			$this->clearSQL();
			$this->appendSQL("SELECT a.rate_code, b.name, c.sdate, c.edate, c.price FROM $this->preferenceTableName a ");
			...
			while (!$this->EOF) {
				...
				$this->preRateSDate[$cnt] = $this->getField("sdate");
				$this->preRateEDate[$cnt] = $this->getField("edate");
		...
		function getRoomList() {
			$this->clearSQL();
			$this->appendSQL("SELECT room_code FROM $this->roomTableName WHERE rate_code LIKE 'G_%' ORDER BY room_code");
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
		...
		function getCalendarList($sdt, $edt) {
			$this->clearSQL();
		...
		function getExcelList($sdate, $edate, $stype, $stext, $rate, $state, $grade, $kind, $current, $period, $sort) {
			...
			$this->clearSQL();
			$this->appendSQL("SELECT a.apply_no, info_no, state, e.account, e.kind, e.fname, e.mname, e.lname, e.prefer, e.name_kr, e.gender, e.dob, e.nationality, e.email, e.phone, e.cell, ");
			...
			$this->appendSQL("a.period_code, b.name, sdate, edate, roommate, room_prefer, a.rate_code, a.admin, SUM(d.price) pay_pr ");
		...
		function getCheckinDate($rate, $period) {
			$returnValue = "0000-00-00";
			$this->clearSQL();
			$this->appendSQL("SELECT sdate FROM $this->priceTableName WHERE rate_code='$rate' AND period_code='$period'");
			$this->parseQuery();
		...
		function getCheckoutDate($rate, $period) {
			$returnValue = "0000-00-00";
			$this->clearSQL();
			$this->appendSQL("SELECT edate FROM $this->priceTableName WHERE rate_code='$rate' AND period_code='$period'");
			$this->parseQuery();
		...
		function insertApplication1($no, $email, $state, $acc, $room_prefer, $mate_nm, $mate_id, $mate_dob, $pre1, $pre2, $pre3, $pre4, $pre5, $pre6, $checkin, $checkout, $period, $rate, $room, $roommate, $admin) {
			global $ihouse_admin_info;
			if (!checkdate((int)substr($checkin, 5, 2), (int)substr($checkin, 8, 2), (int)substr($checkin, 0, 4))) $checkin = $this->getCheckinDate($rate, $period);
			if (!checkdate((int)substr($checkout, 5, 2), (int)substr($checkout, 8, 2), (int)substr($checkout, 0, 4))) $checkout = $this->getCheckoutDate($rate, $period);
			$this->clearSQL();
		...
		function copyApplication($no, $checkin, $checkout, $rate, $period) {
			global $ihouse_admin_info;
			if (!checkdate((int)substr($checkin, 5, 2), (int)substr($checkin, 8, 2), (int)substr($checkin, 0, 4))) $checkin = $this->getCheckinDate($rate, $period);
			if (!checkdate((int)substr($checkout, 5, 2), (int)substr($checkout, 8, 2), (int)substr($checkout, 0, 4))) $checkout = $this->getCheckoutDate($rate, $period);
