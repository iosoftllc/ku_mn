<?
	...
	class cApplication extends cStudent {
		...
		var $calendarRateCode1 = array();
		var $errorMessage;

		function cApplication($host, $id, $pw, $db, $tbl1, $tbl2, $tbl3, $tbl4, $tbl5, $tbl6, $tbl7, $tbl8, $tbl9, $tbl10, $tbl11, $tbl12) {
			$this->rateTableName = $tbl3;
			$this->periodTableName = $tbl4;
			$this->roomTableName = $tbl5;
			$this->priceTableName = $tbl6;
			$this->preferenceTableName = $tbl7;
			$this->paymentTableName = $tbl8;
			$this->refundTableName = $tbl10;
			$this->cStudent($host, $id, $pw, $db, $tbl1, $tbl2, $tbl9, $tbl11, $tbl12);
		}
		...
		function getDormitoryValue1($val){
			if ($val == "IFRH_CJ") $returnValue = "교원동 - CJ Int.House";
			else if ($val == "ISRH_CJ") $returnValue = "학생동 - CJ Int. House";
			else if ($val == "IFRH_FH") $returnValue = "교원동 - Int. Fac. Housing";
			else if ($val == "ISRH_AN") $returnValue = "학생동 - Anam";
			else if ($val == "J_IHOUSE") $returnValue = "CJ Int. House/ANAM Int. House";
			else $returnValue = "";
			return $returnValue;
		}
		...
		function getApplicationList($sdate, $edate, $start, $size, $stype, $stext, $rate, $state, $grade, $kind, $current, $period, $sort) {
			...
			$this->clearSQL();
			$this->appendSQL("SELECT a.apply_no, info_no, e.student_id, e.fname, e.mname, e.lname, e.gender, e.dob, e.email, apply_date, e.nationality, e.current, state, b.name, ");
			$this->appendSQL("a.room_code, f.rate_code, g.name, SUM(price) pay_pr " . $this->getApplicationCondition($sdate, $edate, $stype, $stext, $rate, $state, $grade, $kind, $current, $period) . $sort . $limit);
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
		...
		function getRateList($period) {
			$this->clearSQL();
			$this->appendSQL("SELECT a.rate_code, a.dorm_code, a.name rate_nm, price FROM $this->rateTableName a LEFT JOIN $this->priceTableName b ON a.rate_code=b.rate_code ");
		...
		function getRateList1($dorm="") {
			if ($dorm) $dorm = "WHERE dorm_code LIKE '" . $dorm . "%'";
			$this->clearSQL();
			$this->appendSQL("SELECT rate_code, dorm_code, name FROM $this->rateTableName $dorm ORDER BY dorm_code DESC, order_no");
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
				$this->rateCode[$cnt] = $this->getField("rate_code");
		...
		function getRateList2($dorm="") {
			...
			while (!$this->EOF) {
				$this->rateCode[$cnt] = $this->getField("rate_code");
				$this->rateDormitory[$cnt] = $this->getField("dorm_code");
				$this->rateName[$cnt] = $this->getField("unit");
				$this->setNextRecord();
				$cnt++;
			}
			$this->freeResult();
		...
		function getCalendarList($rate, $sdt, $edt, $dorm="") {
			$this->clearSQL();
			$this->appendSQL("SELECT a.apply_no, a.room_code, a.checkin_dt, a.checkout_dt FROM $this->applyTableName a ");
			$this->appendSQL("LEFT JOIN $this->rateTableName b ON a.rate_code=b.rate_code ");
			$this->appendSQL("WHERE a.rate_code IN ('SINGLE','DOUBLE1','ANAM','TRIPLE') AND a.state NOT IN ('CC','CR','CA') AND a.room_code<>'' AND ");
			$this->appendSQL("((a.checkin_dt>='$sdt' AND a.checkin_dt<='$edt') OR (a.checkout_dt>'$sdt' AND a.checkout_dt<='$edt') OR (a.checkin_dt<'$sdt' AND a.checkout_dt>'$edt')) ");
			if ($rate) $this->appendSQL("AND a.rate_code='$rate' ");
			if ($dorm) $this->appendSQL("AND b.dorm_code LIKE '" . $dorm . "%' ");
			$this->appendSQL("ORDER BY a.room_code, a.checkin_dt");
			$this->parseQuery();
			$cnt = 0; 
			while (!$this->EOF) {
		...
		*/
		function getRoomOldList($rate, $dorm="") {
			$where = "WHERE a.rate_code IN ('SINGLE','DOUBLE1','ANAM','TRIPLE')";
			if ($rate) $where .= " AND a.rate_code='$rate'";
			if ($dorm) $where .= " AND dorm_code LIKE '" . $dorm . "%'";
			$this->clearSQL();
			$this->appendSQL("SELECT a.room_code, a.rate_code FROM $this->roomTableName a LEFT JOIN $this->rateTableName b ON a.rate_code=b.rate_code $where ORDER BY room_code");
			$this->parseQuery();
			$cnt = 0;
			while (!$this->EOF) {
		...
		function getCalendarOldList($rate, $sdt, $edt, $dorm="") {
			$this->clearSQLArray(0);
			$this->clearSQLArray(1);
			$this->appendSQLArray(0, "SELECT a.apply_no, a.room_code, a.checkin_dt, a.checkout_dt FROM $this->applyTableName a ");
			$this->appendSQLArray(0, "LEFT JOIN $this->rateTableName b ON a.rate_code=b.rate_code ");
			$this->appendSQLArray(0, "WHERE a.rate_code IN ('SINGLE','DOUBLE1','ANAM','TRIPLE') AND a.state NOT IN ('CC','CR','CA') AND a.room_code<>'' AND ");
			$this->appendSQLArray(0, "((a.checkin_dt>='$sdt' AND a.checkin_dt<='$edt') OR (a.checkout_dt>'$sdt' AND a.checkout_dt<='$edt') OR (a.checkin_dt<'$sdt' AND a.checkout_dt>'$edt')) ");
			if ($rate) $this->appendSQLArray(0, "AND a.rate_code='$rate' ");
			if ($dorm) $this->appendSQLArray(0, "AND b.dorm_code LIKE '" . $dorm . "%' ");
			$this->appendSQLArray(0, "ORDER BY a.room_code, a.checkin_dt");
			$this->parseQueryArray(0);
			$this->appendSQLArray(1, "SELECT a.apply_no, a.room_code, b.arrival, b.departure FROM $this->bookTableName a ");
			$this->appendSQLArray(1, "LEFT JOIN $this->facultyTableName b ON a.apply_no=b.apply_no LEFT JOIN $this->rateTableName c ON b.rate_code=c.rate_code ");
			$this->appendSQLArray(1, "WHERE b.state IN ('AS','PR','PD') AND ");
			$this->appendSQLArray(1, "((b.arrival>='$sdt' AND b.arrival<='$edt') OR (b.departure>'$sdt' AND b.departure<='$edt') OR (b.arrival<'$sdt' AND b.departure>'$edt')) ");
			if ($rate) $this->appendSQLArray(1, "AND b.rate_code='$rate' ");
			if ($dorm) $this->appendSQLArray(1, "AND c.dorm_code LIKE '" . $dorm . "%' ");
			$this->appendSQLArray(1, "ORDER BY a.room_code, b.arrival");
			$this->parseQueryArray(1);
			$cnt = 0; 
			while (!$this->arrEOF[0]) {
				$this->calendarApply[$cnt] = $this->getFieldArray(0, "apply_no");
				$this->calendarKind[$cnt] = "student";
				$this->calendarRoom[$cnt] = $this->getFieldArray(0, "room_code");
				$this->calendarCheckin[$cnt] = $this->getFieldArray(0, "checkin_dt");
				$this->calendarCheckout[$cnt] = $this->getFieldArray(0, "checkout_dt");
				$this->setNextRecordArray(0);
				$cnt++;
			}
			while (!$this->arrEOF[1]) {
				$this->calendarApply[$cnt] = $this->getFieldArray(1, "apply_no");
				$this->calendarKind[$cnt] = "faculty";
				$this->calendarRoom[$cnt] = $this->getFieldArray(1, "room_code");
				$this->calendarCheckin[$cnt] = $this->getFieldArray(1, "arrival");
				$this->calendarCheckout[$cnt] = $this->getFieldArray(1, "departure");
				$this->setNextRecordArray(1);
				$cnt++;
			}
			$this->freeResultArray(0);
			$this->freeResultArray(1);
		}
		function getRoomList($rate, $dorm="") {
			$where = "WHERE a.rate_code IN ('J_SINGLE','J_DOUBLE')";
			if ($dorm != "") {
				if ($rate != "") $where .= " AND a.rate_code='" . $dorm . "_" . $rate . "'";
				else $where .= " AND a.rate_code LIKE '" . $dorm . "\_%'";
			}
			$this->clearSQL();
			$this->appendSQL("SELECT a.room_code, a.rate_code FROM $this->roomTableName a LEFT JOIN $this->rateTableName b ON a.rate_code=b.rate_code $where ORDER BY room_code");
			$this->parseQuery();
		...
		function getCalendarList($rate, $sdt, $edt, $dorm="") {
			$this->clearSQLArray(0);
			$this->clearSQLArray(1);
			$this->appendSQLArray(0, "SELECT a.apply_no, a.room_code, a.checkin_dt, a.checkout_dt, a.rate_code FROM $this->applyTableName a ");
			$this->appendSQLArray(0, "LEFT JOIN $this->rateTableName b ON a.rate_code=b.rate_code ");
			$this->appendSQLArray(0, "WHERE a.rate_code IN ('J_SINGLE','J_DOUBLE') AND a.state NOT IN ('CC','CR','CA') AND a.room_code<>'' AND ");
			$this->appendSQLArray(0, "((a.checkin_dt>='$sdt' AND a.checkin_dt<='$edt') OR (a.checkout_dt>'$sdt' AND a.checkout_dt<='$edt') OR (a.checkin_dt<'$sdt' AND a.checkout_dt>'$edt')) ");
			if ($dorm != "") {
				if ($rate != "") $this->appendSQLArray(0, "AND a.rate_code='" . $dorm . "_" . $rate . "' ");
				else $this->appendSQLArray(0, "AND a.rate_code LIKE '" . $dorm . "\_%' ");
			}
			$this->appendSQLArray(0, "ORDER BY a.room_code, a.checkin_dt");
			$this->parseQueryArray(0);
			$this->appendSQLArray(1, "SELECT a.apply_no, a.room_code, b.arrival, b.departure, b.rate_code FROM $this->bookTableName a ");
			$this->appendSQLArray(1, "LEFT JOIN $this->facultyTableName b ON a.apply_no=b.apply_no LEFT JOIN $this->rateTableName c ON b.rate_code=c.rate_code ");
			$this->appendSQLArray(1, "WHERE b.state IN ('AS','PR','PD') AND ");
			$this->appendSQLArray(1, "((b.arrival>='$sdt' AND b.arrival<='$edt') OR (b.departure>'$sdt' AND b.departure<='$edt') OR (b.arrival<'$sdt' AND b.departure>'$edt')) ");
			if ($dorm != "") {
				if ($rate != "") $this->appendSQLArray(1, "AND b.rate_code='" . $dorm . "_" . $rate . "' ");
				else $this->appendSQLArray(1, "AND b.rate_code LIKE '" . $dorm . "\_%' ");
			}
			$this->appendSQLArray(1, "ORDER BY a.room_code, b.arrival");
			$this->parseQueryArray(1);
			$cnt = 0; 
			while (!$this->arrEOF[0]) {
				...
				$this->calendarCheckin[$cnt] = $this->getFieldArray(0, "checkin_dt");
				$this->calendarCheckout[$cnt] = $this->getFieldArray(0, "checkout_dt");
				$this->calendarRateCode[$cnt] = $this->getFieldArray(0, "rate_code");
				$this->setNextRecordArray(0);
				$cnt++;
			}
			while (!$this->arrEOF[1]) {
				...
				$this->calendarCheckin[$cnt] = $this->getFieldArray(1, "arrival");
				$this->calendarCheckout[$cnt] = $this->getFieldArray(1, "departure");
				$this->calendarRateCode[$cnt] = $this->getFieldArray(1, "rate_code");
				$this->setNextRecordArray(1);
				$cnt++;
			}
			$this->freeResultArray(0);
		...
		function getCalendarList1($rate, $sdt, $edt, $dorm="") {
			$this->clearSQL();
			$this->appendSQL("SELECT a.apply_no, a.room_code, b.arrival, b.departure, b.rate_code FROM $this->bookTableName a ");
			...
			$this->appendSQL("WHERE b.state IN ('AS','PR','PD') AND ");
			$this->appendSQL("((b.arrival>='$sdt' AND b.arrival<='$edt') OR (b.departure>'$sdt' AND b.departure<='$edt') OR (b.arrival<'$sdt' AND b.departure>'$edt')) ");
			//if ($rate) $this->appendSQL("AND b.rate_code IN ($rate) ");
			//if ($dorm) $this->appendSQL("AND c.dorm_code LIKE '" . $dorm . "%' ");
			if ($dorm != "") {
				if ($rate != "") $this->appendSQLArray("AND b.rate_code='" . $dorm . "_" . $rate . "' ");
				else $this->appendSQLArray("AND b.rate_code LIKE '" . $dorm . "\_%' ");
			}
			$this->appendSQL("ORDER BY a.room_code, b.arrival");
			$this->parseQuery();
			$cnt = 0; 
			while (!$this->EOF) {
				$this->calendarApply1[$cnt] = $this->getField("apply_no");
				$this->calendarRoom1[$cnt] = $this->getField("room_code");
				$this->calendarCheckin1[$cnt] = $this->getField("arrival");
				$this->calendarCheckout1[$cnt] = $this->getField("departure");
				$this->calendarRateCode1[$cnt] = $this->getField("rate_code");
				$this->setNextRecord();
				$cnt++;
			}
			$this->freeResult();
		...
		function getExcelList($sdate, $edate, $stype, $stext, $rate, $state, $grade, $kind, $current, $period, $sort) {
			...
			$this->clearSQL();
			$this->appendSQL("SELECT a.apply_no, info_no, state, e.account, e.kind, e.fname, e.mname, e.lname, e.prefer, e.name_kr, e.gender, e.dob, e.nationality, e.kgsp, e.email, e.phone, e.cell, ");
			$this->appendSQL("apply_date, checkin_dt, checkout_dt, dorm_waived, e.student_id, e.major, e.class, e.home_uni, e.home_addr, e.home_addr1, e.home_addr2, e.home_city, e.home_state, e.home_postal, e.home_country, e.current, mate_name, mate_id, mate_dob, match_local, match_nosmoke, ");
			...
			$this->appendSQL("a.period_code, b.name, sdate, edate, roommate, room_prefer, a.rate_code, a.admin, SUM(price) pay_pr ");
			...
			while (!$this->EOF) {
				...
				$this->listDormWaived[$cnt] = $this->getField("dorm_waived");
				$this->listStudentID[$cnt] = $this->getField("student_id");
		...
		function getExcelList1($sdate, $edate, $stype, $stext, $rate, $state, $grade, $kind, $current, $period, $sort) {
			...
			$this->clearSQL();
			$this->appendSQL("SELECT a.apply_no, e.account, a.period_code, a.rate_code, a.room_code, e.student_id, e.lname, e.fname, e.mname, pay_dt, pay_kind, price ");
		...
		function getApplicationInfo($no) {
			...
			$this->appendSQL("LEFT JOIN $this->roomTableName c ON a.room_code=c.room_code LEFT JOIN $this->studentTableName d ON a.email=d.email LEFT JOIN $this->rateTableName e ON a.rate_code=e.rate_code WHERE apply_no=$no");
			$this->parseQuery();
			if (!$this->EOF) {
				...
				$this->applyDormWaived = $this->getField("dorm_waived");
				...
				if ($this->getField("home_addr1")) $this->personHomeAddress .= ", " . $this->getField("d.home_addr1");
				if ($this->getField("home_addr2")) $this->personHomeAddress .= ", " . $this->getField("d.home_addr2");
				if ($this->getField("home_city")) $this->personHomeAddress .= ", " . $this->getField("d.home_city");
				if ($this->getField("home_state")) $this->personHomeAddress .= ", " . $this->getField("d.home_state");
				if ($this->getField("home_country")) $this->personHomeAddress .= ", " . $this->getField("d.home_country");
				if ($this->getField("home_postal")) $this->personHomeAddress .= " [" . $this->getField("d.home_postal") . "]";
		...
		function getApplicationInfo1($student) {
			$this->clearSQL();
			$this->appendSQL("SELECT * FROM $this->applyTableName a LEFT JOIN $this->periodTableName b ON a.period_code=b.period_code ");
			$this->appendSQL("LEFT JOIN $this->roomTableName c ON a.room_code=c.room_code LEFT JOIN $this->studentTableName d ON a.email=d.email ");
			$this->appendSQL("WHERE d.student_id='$student' and (a.period_code='2006SA' or a.period_code='2006SB')");
			$this->parseQuery();
			if (!$this->EOF) {
				$this->applyNumber = $this->getField("apply_no");
				$this->applyState = $this->getField("state");
		...
		function insertApplication($no, $email, $state, $acc, $room_prefer, $mate_nm, $mate_id, $mate_dob, $pre1, $pre2, $pre3, $pre4, $pre5, $pre6, $checkin, $checkout, $dorm_waived, $period, $stt1, $stt2, $stt3, $stt4, $roommate, $admin) {
			...
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->applyTableName (apply_no, email, state, account, room_prefer, mate_name, mate_id, mate_dob, match_nosmoke, match_bed, match_getup, match_silence, match_study, match_local, apply_date, checkin_dt, checkout_dt, dorm_waived, period_code, settle1, settle2, settle3, settle4, roommate, admin)");
			$this->appendSQL("VALUES ('$no', '$email', '$state', '$acc', '$room_prefer', '$mate_nm', '$mate_id', '$mate_dob', '$pre1', '$pre2', '$pre3', '$pre4', '$pre5', '$pre6', now(), '$checkin', '$checkout', '$dorm_waived', '$period', '$stt1', '$stt2', '$stt3', '$stt4', '$roommate', '$admin')");
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'S', 'A', 'N', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			return $returnValue;
		...
		function copyApplication($no, $checkin, $checkout, $period) {
			...
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->applyTableName (apply_no, email, room_prefer, mate_name, mate_id, mate_dob, match_nosmoke, match_bed, match_getup, match_silence, match_study, match_local, apply_date, room_code, checkin_dt, checkout_dt, dorm_waived, period_code, roommate)");
			$this->appendSQL("VALUES ('$app_no', '" . $this->personEmail . "', '" . $this->roomPrefer . "', '" . $this->mateName . "', '" . $this->mateID . "', '" . $this->mateBirthDate . "', '" . $this->matchNonSmoker . "', '" . $this->matchBedEarly . "', '" . $this->matchGetupEarly . "', '" . $this->matchSilenceStudy . "', '" . $this->matchDayStudy . "', '" . $this->matchLocal . "', now(), '" . $this->linkRoomCode . "', '$checkin', '$checkout', " . $this->applyDormWaived . "'$period', '" . $this->applyRoommate . "')");
			$returnValue = $this->execQuery();
			if ($returnValue) {
				$this->applyNumber = $this->getInsertID();
		...
		function updateApplication($no, $email, $rate, $state, $acc, $room_prefer, $mate_nm, $mate_id, $mate_dob, $pre1, $pre2, $pre3, $pre4, $pre5, $pre6, $checkin, $checkout, $dorm_waived, $period, $stt1, $stt2, $stt3, $stt4, $roommate, $admin) {
			global $ihouse_admin_info;
			if (!checkdate((int)substr($checkin, 5, 2), (int)substr($checkin, 8, 2), (int)substr($checkin, 0, 4))) $checkin = $this->getCheckinDate($period);
			if (!checkdate((int)substr($checkout, 5, 2), (int)substr($checkout, 8, 2), (int)substr($checkout, 0, 4))) $checkout = $this->getCheckoutDate($period);
			$this->clearSQL();
			...
			$this->appendSQL("match_study='$pre5', match_local='$pre6', checkin_dt='$checkin', checkout_dt='$checkout', dorm_waived='$dorm_waived', period_code='$period', settle1='$stt1', settle2='$stt2', settle3='$stt3', settle4='$stt4', ");
			...
			if ($returnValue) {
				...
				if (strtolower($admin_id) != "intia") {
					...
					$this->clearSQL();
					$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'S', 'A', 'E', '$ip', now(), '$detail')");
					$this->execQuery();
				}
			}
			return $returnValue;
		...
		function updateApplicationState1($student, $state) {
			$returnValue = false;
			if ($student && $state) {
				$apply_no = "";
				$this->clearSQL();
				$this->appendSQL("SELECT apply_no FROM $this->studentTableName a LEFT JOIN $this->applyTableName b ON a.email=b.email WHERE period_code='2007FA' AND a.student_id='$student'");
				$this->parseQuery();
				if (!$this->EOF) $apply_no = $this->getField("apply_no");
				$this->freeResult();
				if ($apply_no) {
					$this->clearSQL();
					$this->appendSQL("UPDATE $this->applyTableName SET state='$state' WHERE apply_no=$apply_no");
					$returnValue = $this->execQuery();
				}
			}
			return $returnValue;
		...
		function isRoomAvailable($no, $room) {
			...
			if ($arrival && $departure) {
				...
				$this->appendSQL("LEFT JOIN $this->facultyTableName b ON a.apply_no=b.apply_no LEFT JOIN $this->rate1TableName c ON b.rate_code=c.rate_code ");
				$this->appendSQL("WHERE a.room_code='$room' AND ((arrival<='$arrival' AND departure>'$arrival') OR ");
		...
		function assignRoom($no, $room) {
			...
			if ($room) {
				$this->clearSQL();
				$this->appendSQL("UPDATE $this->applyTableName SET assign_date=now(), state='RA', room_code='$room' WHERE apply_no=$no");
				$returnValue = $this->execQuery();
			}
			return $returnValue;
		}

		function assignRoom1($no, $rate, $room) {
			$returnValue = false;
			if ($room) {
				$this->clearSQL();
				$this->appendSQL("UPDATE $this->applyTableName SET assign_date=now(), state='RA', rate_code='$rate', room_code='$room' WHERE apply_no=$no");
				$returnValue = $this->execQuery();
			}
			return $returnValue;
		}

		function assignRoom2($no, $room) {
			$returnValue = false;
			$this->linkRateCode = "";
			if ($room) {
				$this->clearSQL();
				...
				if ($returnValue) {
					...
					if ($returnValue) {
						...
						if (strtolower($admin_id) != "intia") {
							...
							$this->clearSQL();
							$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'S', 'A', 'D', '$ip', now(), '$detail')");
							$this->execQuery();
						}
					}
				}
		...
		function updateNationality($apply, $nation) {
			global $ihouse_admin_info;
			$returnValue = true;
			if ($apply && $nation) {
				$email = "";
				$this->clearSQL();
				$this->appendSQL("SELECT email FROM $this->applyTableName WHERE apply_no='$apply'");
				$this->parseQuery();
				if (!$this->EOF) $email = $this->getField("email");
				$this->freeResult();
				if ($email) {
					$this->clearSQL();
					$this->appendSQL("UPDATE $this->studentTableName SET nationality='$nation' WHERE email='$email'");
					$returnValue = $this->execQuery();
					if ($returnValue) {
						$admin_id = $ihouse_admin_info[id];
						if (strtolower($admin_id) != "intia") {
							$ip = $_SERVER["REMOTE_ADDR"];
							$detail = "$apply 온라인지원 지원자 국적 수정";
							$this->clearSQL();
							$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'S', 'A', 'E', '$ip', now(), '$detail')");
							$this->execQuery();
						}
		...
		function updateNationality1($student, $nation) {
			global $ihouse_admin_info;
			$returnValue = true;
			if ($student && $nation) {
				/*
				$email = "";
				$this->clearSQL();
				$this->appendSQL("SELECT email FROM $this->applyTableName WHERE student_id='$student'");
				$this->parseQuery();
				if (!$this->EOF) $email = $this->getField("email");
				$this->freeResult();
				if ($email) {
				...
				*/
				$this->clearSQL();
				$this->appendSQL("UPDATE $this->studentTableName SET nationality='$nation' WHERE student_id='$student'");
				$returnValue = $this->execQuery();
				if ($returnValue) {
					$admin_id = $ihouse_admin_info[id];
					if (strtolower($admin_id) != "intia") {
						$ip = $_SERVER["REMOTE_ADDR"];
						$detail = "$student 학번 지원자 국적 수정";
						$this->clearSQL();
						$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'S', 'A', 'E', '$ip', now(), '$detail')");
						$this->execQuery();
					}
		...
		function updateStudentIDPW($old, $new, $pw) {
			global $ihouse_admin_info;
			$returnValue = false;
			if ($old && $new && $pw) {
				$student_no = "";
				$this->clearSQL();
				$this->appendSQL("SELECT info_no FROM $this->studentTableName a, $this->applyTableName b WHERE a.email=b.email AND period_code='2007ISC' AND a.student_id='$old'");
				$this->parseQuery();
				if (!$this->EOF) $student_no = $this->getField("info_no");
				$this->freeResult();
				if ($student_no) {
					$this->clearSQL();
					$this->appendSQL("UPDATE $this->studentTableName SET student_id='$new', passwd=encrypt('$pw') WHERE info_no='$student_no'");
					$returnValue = $this->execQuery();
					if ($returnValue) {
						$admin_id = $ihouse_admin_info[id];
						if (strtolower($admin_id) != "intia") {
							$ip = $_SERVER["REMOTE_ADDR"];
							$detail = "$old 학번에서 $new 학번으로 변경 및 비밀번호 변경 ($student_no)";
							$this->clearSQL();
							$this->appendSQL("INSERT INTO $this->historyWorkTableName (mem_id, building, menu, kind, work_ip, work_time, detail) VALUES ('$admin_id', 'S', 'A', 'E', '$ip', now(), '$detail')");
							$this->execQuery();
						}
		...
		function updateAccountFee($student, $amount, $dt) {
			$apply = "";
			$this->clearSQL();
			$this->appendSQL("SELECT apply_no FROM $this->applyTableName a, $this->studentTableName b WHERE a.email=b.email AND b.student_id='$student' AND period_code='2007ISC'");
			$this->parseQuery();
			if (!$this->EOF) $apply = $this->getField("apply_no");
			$this->freeResult();
			if ($apply) $returnValue = $this->insertPayment($apply, "E", $dt, "-".$amount, "RF");
			else $returnValue = false;
			if ($returnValue) $this->updateApplicationState($apply, "FD");
			return $returnValue;
		}

		function updateAccountFee1($account, $amount, $dt) {
			$apply = "";
			$this->clearSQL();
			$this->appendSQL("SELECT apply_no FROM $this->studentTableName a LEFT JOIN $this->applyTableName b ON a.email=b.email WHERE REPLACE(a.account,'-','')='$account' AND period_code='2007FA';");
			$this->parseQuery();
			if (!$this->EOF) $apply = $this->getField("apply_no");
			$this->freeResult();
			if ($apply) $returnValue = $this->insertPayment($apply, "E", $dt, "-".$amount, "DF");
			else $returnValue = false;
			if ($returnValue) $this->updateApplicationState($apply, "DD");
			return $returnValue;
		}

		function updateAccountFee2($account, $amount, $dt) {
			$apply = "";
			$this->clearSQL();
			$this->appendSQL("SELECT apply_no FROM $this->studentTableName a LEFT JOIN $this->applyTableName b ON a.email=b.email WHERE REPLACE(a.account,'-','')='$account' AND period_code='2007FA';");
			$this->parseQuery();
		...
		function updateAccountFee3($apply, $amount, $dt, $kind) {
			if ($apply) $returnValue = $this->insertPayment($apply, "E", $dt, $amount, $kind);
			else $returnValue = false;
			return $returnValue;
		}

		function deleteAccountFee($apply) {
			$this->clearSQL();
			$this->appendSQL("DELETE FROM $this->paymentTableName WHERE pay_kind IN ('DP','DF') AND apply_no=$apply");
			$returnValue = $this->execQuery();
			return $returnValue;
		}
		...
		function updateAccountFlag($student) {
			$apply = "";
			$this->clearSQL();
			$this->appendSQL("SELECT apply_no FROM $this->applyTableName a, $this->studentTableName b WHERE a.email=b.email AND b.student_id='$student' AND (period_code='2006SA' OR period_code='2006SB')");
			$this->parseQuery();
			if (!$this->EOF) $apply = $this->getField("apply_no");
			$this->freeResult();
			if ($apply) $returnValue = $this->updateApplicationState($apply, "PW");
			else $returnValue = false;
			return $returnValue;
		}
		...
		function insertPayment1($student, $kind, $dt, $pr, $detail) {
			$returnValue = false;
			$this->clearSQL();
			$this->appendSQL("SELECT apply_no FROM $this->applyTableName a, $this->studentTableName b WHERE a.email=b.email AND b.student_id='$student' and period_code='2006FA'");
			$this->parseQuery();
			if (!$this->EOF) {
		...
		function insertPayment2($no, $kind, $dt, $pr, $detail) {
			$returnValue = false;
			if (is_numeric($no) && $no > 0) {
				$this->clearSQL();
				$this->appendSQL("INSERT INTO $this->paymentTableName (apply_no, kind, pay_dt, price, pay_kind) VALUES ('$no', '$kind', '$dt', $pr, '$detail')");
				$returnValue = $this->execQuery();
			}
			return $returnValue;
		}
		...
		function insertRefund($no, $cf_no, $kind, $student, $fname, $mname, $lname, $dob, $email, $dorm, $room, $old, $new) {
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->refundTableName (apply_no, cf_apply_no, kind, student_id, fname, mname, lname, dob, email, post_dt, dorm_code, room_code, old_period, new_period)");
			$this->appendSQL("VALUES ('$no', '$cf_no', '$kind', '$student', '$fname', '$mname', '$lname', '$dob', '$email', now(), '$dorm', '$room', '$old', '$new')");
			$returnValue = $this->execQuery();
			if ($returnValue) {
				$this->applyNumber = $this->getInsertID();
				if ($no) {
