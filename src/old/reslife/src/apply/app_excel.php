<?
	...
	if ($log_flag) {
		if ((int)$ihouse_admin_info[grade] < 8) include_once("../common/check_auth_tpl.php");
			...
			//$record = "지원번호,지원상태,학생종류,재사여부,지원일,입실일,퇴실일,보증금납부일,가상계좌,미납액,고대학번,성,이름,애칭,영문성명,한글성명,성별,생년월일,국적,모교,학과,과정,KGSP,Dormitory Fee fully/partially waived,이메일,";
			...
			for ($i = 0; $i < count($application->listNumber); $i++) {
				...
				if ($application->preRateCode[0]) $rate1 = $application->getDormitoryValue($application->preRateDormitory[0], $application->preRateCode[0]) . ": " . $application->preRateName[0] . " " . number_format($application->preRatePrice[0]) . "KRW";
				else $rate1 = "";
				if ($application->preRateCode[1]) $rate2 = $application->getDormitoryValue($application->preRateDormitory[1], $application->preRateCode[1]) . ": " . $application->preRateName[1] . " " . number_format($application->preRatePrice[1]) . "KRW";
				else $rate2 = "";
				if ($application->preRateCode[2]) $rate3 = $application->getDormitoryValue($application->preRateDormitory[2], $application->preRateCode[2]) . ": " . $application->preRateName[2] . " " . number_format($application->preRatePrice[2]) . "KRW";
				else $rate3 = "";
				if ($application->preRateCode[3]) $rate4 = $application->getDormitoryValue($application->preRateDormitory[3], $application->preRateCode[3]) . ": " . $application->preRateName[3] . " " . number_format($application->preRatePrice[3]) . "KRW";
				else $rate4 = "";
				if ($application->preRateCode[4]) $rate5 = $application->getDormitoryValue($application->preRateDormitory[4], $application->preRateCode[4]) . ": " . $application->preRateName[4] . " " . number_format($application->preRatePrice[4]) . "KRW";
				else $rate5 = "";
				if ($application->preRateCode[5]) $rate6 = $application->getDormitoryValue($application->preRateDormitory[5], $application->preRateCode[5]) . ": " . $application->preRateName[5] . " " . number_format($application->preRatePrice[5]) . "KRW";
				else $rate6 = "";
				if ($application->preRateCode[6]) $rate7 = $application->getDormitoryValue($application->preRateDormitory[6], $application->preRateCode[6]) . ": " . $application->preRateName[6] . " " . number_format($application->preRatePrice[6]) . "KRW";
				else $rate7 = "";
				if ($application->preRateCode[7]) $rate8 = $application->getDormitoryValue($application->preRateDormitory[7], $application->preRateCode[7]) . ": " . $application->preRateName[7] . " " . number_format($application->preRatePrice[7]) . "KRW";
				else $rate8 = "";
				$sel_rate = "";
				...
				else if ($application->preRateCode[3] == $application->listRateCode[$i]) $sel_rate = $rate4;
				else if ($application->preRateCode[4] == $application->listRateCode[$i]) $sel_rate = $rate5;
				else if ($application->preRateCode[5] == $application->listRateCode[$i]) $sel_rate = $rate6;
				else if ($application->preRateCode[6] == $application->listRateCode[$i]) $sel_rate = $rate7;
				else if ($application->preRateCode[7] == $application->listRateCode[$i]) $sel_rate = $rate8;
				...
				$record .= $gender . "\",\"" . $dob . "\",\"";
				$record .= $application->listNation[$i] . "\",\"";
				...
				//$record .= $application->getPreferenceValue($application->listDormWaived[$i]) . "\",\"";
