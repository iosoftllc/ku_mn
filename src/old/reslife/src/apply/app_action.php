	...
	if ($log_flag) {
		if ((int)$ihouse_admin_info[grade] < 8) include_once("../common/check_auth_tpl.php");
			...
			switch ($mode) {
				case "new":
					...
					if ($fee_transfer_size <= $max_attach && $fee_support_size <= $max_attach && $tb_test_size <= $max_attach) {
						...
						$flag = $application->insertApplication($app_no, $email, $state, $account, $room_prefer, $mate_nm, $mate_id, $mate_dob, $match_pre1, $match_pre2, $match_pre3, $match_pre4, $match_pre5, $match_pre6, $checkin, $checkout, $dorm_waived, $period, $settle1, $settle2, $settle3, $settle4, $roommate, $admin);
						if ($flag) {
							$flag = $application->insertPreference($rate, $application->applyNumber);
							if($flag) {
								//if ($current != "Y") $flag = $application->insertPayment($application->applyNumber, "D", date("Y-m-d"), "200000", "DB");
								//if (!$flag) {
									//$application->deleteApplication($application->applyNumber);
									//$mode = "error";
								//} else $application->approveApplication($application->applyNumber, $ihouse_admin_info[grade], "Y");
				...
				case "edit":
					...
					if ($fee_transfer_size <= $max_attach && $fee_support_size <= $max_attach && $tb_test_size <= $max_attach) {
						...
						if ($current != "Y") $room_prefer = "";
						$flag = $application->updateApplication($no, $email, $sel_rate, $state, $account, $room_prefer, $mate_nm, $mate_id, $mate_dob, $match_pre1, $match_pre2, $match_pre3, $match_pre4, $match_pre5, $match_pre6, $checkin, $checkout, $dorm_waived, $period, $settle1, $settle2, $settle3, $settle4, $roommate, $admin);
						if ($flag) {
							if ($rate) {
				...
				case "assign":
					...
					if ($application->isRoomExist($no, $room)) $mode = "exist";
						...
						$flag = $application->assignRoom2($no, $room);
						if ($flag) {
							//if (!$application->isDepositBilled($no)) $flag = $application->insertPayment($no, "D", date("Y-m-d"), "200000", "DB");
				...
				case "room";
					...
					$msg .= "Find a Hana Bank branch that is closest to your work or home. Bring your billing statement to your branch of choice and pay by cash during the period before <font style='text-decoration:underline;'>July. 18. 2021</font>. Note: In person payment is available between 9:30 am and 4:30 pm on weekdays, except holidays.<br><br>\n";
					$msg .= "<b>Online</b><br>\n";
					$msg .= "Pay by Internet Banking - Electronic Funds Transfer only from a Korean bank account, during the period before <font style='text-decoration:underline;'>July. 18. 2021</font>. Note: Online payment is available between 9:30 am and 4:30 pm on weekdays, except holidays.<br><br>\n";
					$msg .= "If your Dormitory fee is fully/partially waived. Please upload relative document on website before July. 18. 2021<br><br>\n";
				...
				case "upload";
					...
					if (is_uploaded_file($attach)) {
						...
						while ($data = fgetcsv($fp, 1000, ",")) {
							...
							if ($flag) {
								$flag = $application->insertPreference($rate, $application->applyNumber);
								//$flag = $application->insertPayment($application->applyNumber, "D", date("Y-m-d"), "200000", "DB");
				...
				case "force";
					...
					if (!$application->applyNumber) $mode = "no_apply";
						...
						$flag = $application->insertRefund($no, $cf_no, $kind, $student, $fname, $mname, $lname, $dob, $email, $dorm, $room, $old_period, $new_period);
						if (!$flag) $mode = "error";
					}
					break;
			}
			...
			if ($mode == "error") {
				...
				echo "document.location.replace(\"app_view.php?no=$no&page=$page&s_type=$s_type&s_text=$s_text&s_kind=$s_kind&s_rate=$s_rate&s_state=$s_state&s_grade=$s_grade&s_current=$s_current&s_period=$s_period&sort1=$sort1&sort2=$sort2\");";
				echo "</script>";
			} else if ($mode == "upload") {
				echo "<script langauage=\"javascript\">";
			...
			else if ($mode == "add" || $mode == "del") header("Location: app_list.php?page=$page&s_type=$s_type&s_text=$s_text&s_kind=$s_kind&s_rate=$s_rate&s_state=$s_state&s_grade=$s_grade&s_current=$s_current&s_period=$s_period&s_year1=$s_year1&s_month1=$s_month1&s_day1=$s_day1&s_year2=$s_year2&s_month2=$s_month2&s_day2=$s_day2&sort1=$sort1&sort2=$sort2");
