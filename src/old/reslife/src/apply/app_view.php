	...
	if ($log_flag) {
		if ((int)$ihouse_admin_info[grade] != 0 && (int)$ihouse_admin_info[grade] != 1 && (int)$ihouse_admin_info[grade] != 2 && (int)$ihouse_admin_info[grade] != 7 && (int)$ihouse_admin_info[grade] < 8) include_once("../common/check_auth_tpl.php");
			...
			if ($period) {
				...
				for ($i = 0 ; $i < count($application->preRateCode); $i++) {
					...
					if ($application->preRateCode[$i]) {
						$preference = $application->getDormitoryValue($application->preRateDormitory[$i], $application->preRateCode[$i]) . ": " . $application->preRateName[$i] . " " . number_format($application->preRatePrice[$i]) . "KRW";
			...
			$tpl->assign(array(SEL_PAGE           => $page,
			                   ...
			                   APPLY_DORM_WAIVED  => $application->getPreferenceValue($application->applyDormWaived),
