<?
	...
	if ($log_flag) {
		if ((int)$ihouse_admin_info[grade] != 0 && (int)$ihouse_admin_info[grade] != 1 && (int)$ihouse_admin_info[grade] != 2 && (int)$ihouse_admin_info[grade] != 7 && (int)$ihouse_admin_info[grade] < 8) include_once("../common/check_auth_tpl.php");
			...
			$sdate = $sel_dt . "-01";
			$edate = $sel_dt . "-" . $maxday;
			$application->getCalendarOldList($s_rate, $sdate, $edate, $temp_dorm);
			$application->getCalendarList1($s_rate, $sdate, $edate, $temp_dorm);
			$day_list = "";
			$empty_list = "";
			$room_count = array();
			for ($i = 1; $i <= $maxday; $i++) {
			...
			$room_list = "";
			$application->getRoomOldList($s_rate, $temp_dorm);
