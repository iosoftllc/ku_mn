<?
	...
	if ($log_flag) {
		if ((int)$ihouse_admin_info[grade] < 6 || (int)$ihouse_admin_info[grade] == 7 || (int)$ihouse_admin_info[grade] == 8) include_once("../common/check_auth_tpl.php");
			...
			$record = "구분,재사여부,고대학번,성,이름,애칭,한글성명,성별,생년월일,국적,KGSP,모교,홈주소,거주주소,학과,학년,이메일,전화번호,핸드폰번호,";
			...
			for ($i = 0; $i < count($student->stuListNumber); $i++) {
				$nation = $student->stuListNationality[$i];
				if (trim($student->stuListProvince[$i]) != "") $nation .= " - " . $student->stuListProvince[$i];
