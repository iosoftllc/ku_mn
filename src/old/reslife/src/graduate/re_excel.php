<?
	...
	if ($log_flag) {
		if ((int)$ihouse_admin_info[grade] < 6 || (int)$ihouse_admin_info[grade] == 7 || (int)$ihouse_admin_info[grade] == 8) include_once("../common/check_auth_tpl.php");
			...
			$record = "인덱스,학생종류,지원번호,이전지원번호,고대학번,환불종류,승인여부,Deposit Paid,Overpayment,Total Deposit Payment,Damage Deduction,Cancellation Fee,Late Check out,차감액,반환 및 이전 금액,신청체류기간,적용체류기간,룸번호,성,이름,생년월일,이메일,국적,최근수정일,승인일,신청일,퇴실일,환불방법,환불정보1,환불정보2,환불정보3,관리자노트\n";
			...
			for ($i = 0; $i < count($refund->listNumber); $i++) {
				...
				$record .= $refund->getKindValue($refund->listKind[$i]) . "\",\"";
