<?
	...
	switch ($mode) {
		case "apply":
			...
			if ($application->hasSamePeriod($email, $period)) $mode = "period";
				...
				$room = $_POST["room"];
				$rate = $_POST["rate1"] . "," . $_POST["rate2"] . "," . $_POST["rate3"];
				...
				$flag = $application->insertApplication($app_no, $email, $room_prefer, $mate_nm, $mate_id, $mate_dob, $match_pre1, $match_pre2, $match_pre3, $match_pre4, $match_pre5, $match_pre6, $room, $period);
		...
		case "edit":
			...
			if ($application->hasSamePeriod($email, $period) && $no != $application->applyNumber) $mode = "period";
				...
				if ($current != "Y") $room_prefer = "";
				$flag = $application->updateApplication($no, $email, $room_prefer, $mate_nm, $mate_id, $mate_dob, $match_pre1, $match_pre2, $match_pre3, $match_pre4, $match_pre5, $match_pre6, $room, $period);
				if ($flag) {
					if ($rate) {
				...
				if (!$application->applyNumber) $mode = "no_apply";
					...
					*/
					if (!$process_flag) $mode = "exceed";
						...
						$flag = $application->insertRefund($no, $cf_no, $student, $fname, $mname, $lname, $dob, $email, $vacate, $method_type, $method_info1, $method_info2, $method_info3, $method_info4, $method_info5, $method_info6, $addr1, $addr2, $city, $state, $postal, $country, $room, $old_period, $new_period);
						if ($flag) {
	...
	if ($mode == "error") {
		...
		echo "<form name=\"ActionForm\" action=\"../../src/graduate/view_app.php\" method=\"post\">";
		...
		echo "<form name=\"ActionForm\" action=\"../../src/graduate/list_app.php\" method=\"post\">";
		echo "<input type=\"hidden\" name=\"no\" value=\"$no\">";
		echo "<input type=\"hidden\" name=\"email\" value=\"$email\">";
		echo "<input type=\"hidden\" name=\"pw\" value=\"$pw\">";
		echo "</form>";
		...
		echo "<form name=\"ActionForm\" action=\"../../src/graduate/payment.php\" method=\"post\">";
		echo "<input type=\"hidden\" name=\"email\" value=\"$email\">";
		echo "<input type=\"hidden\" name=\"pw\" value=\"$pw\">";
		echo "</form>";
		echo "<script langauage=\"javascript\">";
