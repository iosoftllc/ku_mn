<?
	...
	$tpl->define(array(main => "application_stu.html"));
	...
	if ($period) {
		...
		for ($i = 0 ; $i < count($application->preRateCode); $i++) {
			...
			if ($application->preRateCode[$i]) {
				$preference = $application->getDormitoryValue($application->preRateDormitory[$i]) . ": " . $application->preRateName[$i] . " " . number_format($application->preRatePrice[$i]) . "KRW";
			}
