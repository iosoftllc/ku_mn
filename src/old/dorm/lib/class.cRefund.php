<?
	...
	class cRefund extends cMysql {
		...
		function cRefund($host, $id, $pw, $db, $tbl1, $tbl2, $tbl3) {
		...
		function getApplicantInfo($no) {
			...
			if (!$this->EOF) {
				...
				$this->applyDorm = $this->getField("dorm_code");
		...
		function insertRefund($kind, $student, $fname, $mname, $lname, $dob, $email, $vacate, $method, $info1, $info2, $info3, $info4, $info5, $info6, $dorm, $room, $old, $new) {
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->refundTableName (kind, student_id, fname, mname, lname, dob, email, vacate_dt, post_dt, method_type, method_info1, method_info2, method_info3, method_info4, method_info5, method_info6, dorm_code, room_code, old_period, new_period)");
			$this->appendSQL("VALUES ('$kind', '$student', '$fname', '$mname', '$lname', '$dob', '$email', '$vacate', now(), '$method', '$info1', '$info2', '$info3', '$info4', '$info5', '$info6', '$dorm', '$room', '$old', '$new')");
		...
		function insertRefund1($no, $cf_no, $kind, $student, $fname, $mname, $lname, $dob, $email, $vacate, $method, $info1, $info2, $info3, $info4, $info5, $info6, $dorm, $room, $old, $new) {
			$this->clearSQL();
			$this->appendSQL("INSERT INTO $this->refundTableName (apply_no, cf_apply_no, kind, student_id, fname, mname, lname, dob, email, vacate_dt, post_dt, method_type, method_info1, method_info2, method_info3, method_info4, method_info5, method_info6, dorm_code, room_code, old_period, new_period)");
			$this->appendSQL("VALUES ('$no', '$cf_no', '$kind', '$student', '$fname', '$mname', '$lname', '$dob', '$email', '$vacate', now(), '$method', '$info1', '$info2', '$info3', '$info4', '$info5', '$info6', '$dorm', '$room', '$old', '$new')");
			$returnValue = $this->execQuery();
			if ($returnValue) {
				$this->applyNumber = $this->getInsertID();
				if ($new) {
