...
//
// RCS Revision
//   @(#) $Id: class.rFastTemplate.php,v 1.17 2001/08/08 01:50:35 roland Exp $
...
class rFastTemplate {
   ...
   function parse_internal ($tag) {
      ...
      if (gettype($tag) == 'array') {
	    ...
	    if ($this->TEMPLATE[$tag][dynamic]) {
		  ...
		  reset ($this->TEMPLATE);
		  foreach (array_keys($this->TEMPLATE) as $key => $ptag) {
		     if ($debug)
   ...
   function subst ($handle, $tag, $autoload = true) {
      ...
      if (gettype($tag) == 'array') {
         $count = 0;
         foreach (array_values($tag) as $key => $t) {
            if ($count != 0)
               $t = ".$t";
            $count ++;
            if ($debug)
      ...
      if (isset($this->TEMPLATE[$tag])) {
         ...
         if ($autoload) {
            ...
            foreach (array_keys($this->TEMPLATE) as $key => $t) {
               if ($debug)
      ...
      if (isset($this->TEMPLATE[$tag][part])) {
         ...
         foreach (array_keys($this->TEMPLATE[$tag][part]) as $key => $p) {
	    if ($debug)
   ...
   function unsetkey ($key = '') {
      if (empty($key)) {
         ...
         reset ($key);
         foreach (array_values($key) as $key => $k) {
            unset ($this->VAR[$k]);
         }
      } else {
         unset ($this->VAR[$key]);
