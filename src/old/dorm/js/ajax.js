...
function refreshRoomList(code, gender) {
	var urlValue = "https://reslife.korea.ac.kr:5008/v1/src/apply/select_stu.php?type=rate&code=" + code + "&gender=" + gender;
...
function updateRoomList() {
	...
	for (var i = 0; i < room.length; i++) {
		...
		if (room[i].getElementsByTagName("dorm")[0].childNodes[0] == null) optionObj.appendChild(document.createTextNode(room[i].getElementsByTagName("name")[0].childNodes[0].nodeValue + " - " + room[i].getElementsByTagName("price")[0].childNodes[0].nodeValue));
		else optionObj.appendChild(document.createTextNode(room[i].getElementsByTagName("dorm")[0].childNodes[0].nodeValue + " - " + room[i].getElementsByTagName("name")[0].childNodes[0].nodeValue + " - " + room[i].getElementsByTagName("price")[0].childNodes[0].nodeValue));
		document.getElementById("rate1").appendChild(optionObj);
	}
