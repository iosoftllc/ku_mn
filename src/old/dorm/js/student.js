...
function checkStudentInfo(Form) {
	...
	*/
	if (Form.id.value == "") {
		...
		Form.id.focus();
		return false;
	//} else if (!checkNumber(Form.id.value)) {
		//alert("Korea University Student number only allow a numeric letter.");
		//Form.id.focus();
		//return false;
	}
	if (Form.lname.value == "") {
	...
	if (!Form.sclass[0].checked && !Form.sclass[1].checked && !Form.sclass[2].checked && !Form.sclass[3].checked && !Form.sclass[4].checked && !Form.sclass[5].checked) {
		alert("Please select Undergraduate or Graduate.");
		Form.sclass[0].focus();
		return false;
	}
	if (!Form.kgsp[0].checked && !Form.kgsp[1].checked) {
		alert("Please select KGSP or N/A.");
		Form.kgsp[0].focus();
		return false;
	}
	...
	/*
	if (Form.nation.value.toLowerCase().match("south korea")) {
		if (Form.name_kr.value == "") {
			alert("Please input a Name in Korean.");
			Form.name_kr.focus();
			Form.name_kr.select();
			return false;
		}
	...
	if (Form.kind[1].checked) {
		if (Form.id.value.length != 8) {
			alert("Korean Language & Culture Center Student number must be 8 digits.");
			Form.id.focus();
			return false;
		}
	} else {
		if (Form.id.value.length != 10) {
			alert("Korea University Student number must be 10 digits.");
			Form.id.focus();
			return false;
		}
	}
	if (Form.id.value.substr(0, 6) != "200695") {
...
function checkDuplicateEmail(Form) {
	...
	var URL = "https://reslife.korea.ac.kr:5008/v1/src/popup/check_email.php?email=" + Form.email.value;
