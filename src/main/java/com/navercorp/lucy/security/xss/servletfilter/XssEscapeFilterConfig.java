...
public class XssEscapeFilterConfig {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(XssEscapeFilterConfig.class);
	...
	private boolean addUrlDisableRule(String url, NodeList nodeList) {
		...
		if (!url.isEmpty()) {
			boolean disable = StringUtils.equalsIgnoreCase(((Element) nodeList.item(0)).getAttribute("disable"), "true")
					? true
					: false;
	...
	private Map<String, XssEscapeFilterRule> createRequestParamRuleMap(Element element) {
		...
		for (int i = 0; nodeList.getLength() > 0 && i < nodeList.getLength(); i++) {
			...
			boolean useDefender = StringUtils.equalsIgnoreCase(eachElement.getAttribute("useDefender"), "false") ? false
					: true;
	...
	private void addDefender(String name, String clazz, String[] args) {
		...
		if (StringUtils.isBlank(name) || StringUtils.isBlank(clazz)) {
			String message = String
					.format("The defender's name('%s') or clazz('%s') is empty. This defender is ignored", name, clazz);
			LOGGER.warn(message);
			return;
		}
		try {
	 ...
	 * @return XssEscapeFilterRule
	 */
	private XssEscapeFilterRule checkParamRule(Map<String, XssEscapeFilterRule> urlParamRuleMap, String url,
			String paramName) {
	 ...
	 * @return XssEscapeFilterRule
	 */
	private XssEscapeFilterRule checkPrefixParameter(String paramName, XssEscapeFilterRule paramRule,
			Map<String, XssEscapeFilterRule> urlParamRuleMap) {
