	...
	switch ($(this).attr('id')) {
		...
		case 'gnb-menu-pixel':
			if (!$(this).hasClass('_active')) {
				if (confirm('변경시 작성된 내용이 사라집니다. 계속하시겠습니까?')) {
					$(this).addClass('_active').siblings('li').removeClass('_active');
		...
		case 'gnb-menu-clear':
			if (confirm('삭제하시겠습니까?')) resetMapElView(); // 맵 엘리먼트 삭제
			else return;
			break;
...
function generateCode() {
	...
	if (!confirm('작성이 완료되었습니까?\n\n수정인 경우 기존에 삽입된 이미지맵은 삭제되지 않으니 에디터에서 삭제 후 삽입하시기 바랍니다.\n\n삭제시 <map> 태그까지 제거되었는지 확인해주세요.')) {
		return;
	}
	...
	// 이미지맵 마크업
	var mapname = mtime;
	var gcM = gcAlert + '&lt;img src="' + imgsrc + '" usemap="#<em>mapname' + mapname + '</em>"&gt;' + '<br>' + '&lt;map name="<em>mapname' + mapname + '</em>"&gt;' + '<br>' + gcMloop + '&lt;/map&gt;';
...
function addMapElView(index) {
	$img.append('<div class="grid-box" id="grid-box-' + index + '"><span class="grid-box-cnt">' + (index + 1) + '</span><span class="grid-box-close">&times;</span><span class="grid-box-link">링크 추가</span></div>');
	...
	if (mapEl[index][2]) {
		...
		$('#grid-box-' + index).find('.grid-box-link').text('링크 변경');
	}
}
...
function addLinkToMapEl() {
	...
	if ($('.grid-box._active').hasClass('_added')) {
		...
		$('#pop-addlink .pop-title').text('변경 URL');
		$('#pop-addlink .pop-btn-confirm').text('링크 변경');
		$('#pop-addlink-input').val(urlink);
		$('input[name="pop-addlink-target"][value="'+ target +'"]').prop('checked', true);
	} else {
		$('#pop-addlink .pop-title').text('추가 URL');
		$('#pop-addlink .pop-btn-confirm').text('링크 추가');
...
function setLinkToMapEl() {
	...
	$('.grid-box._active .grid-box-link').text('링크 변경');
	popClose();
}
